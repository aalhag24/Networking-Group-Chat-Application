...
implementation{
    task void processCommand(){
		dbg(COMMAND_CHANNEL, "Task Processs Command.\n");
        if(! call Queue.empty()){
            CommandMsg *msg;
            uint8_t commandID;
            uint8_t* buff;
            ...
            if(!payload){
                ...
                post processCommand();
				dbg(COMMAND_CHANNEL, "Post Processs Command.\n");
                return;
            }
...
/// HERE
			case CMD_CONNECT_SERVER:
				dbg(COMMAND_CHANNEL, "Command Type: Connect to Server\n");
				signal CommandHandler.ConnectingtoServer(buff[0], buff[1], buff[2], buff[3], &buff[4]);
				break;

			case CMD_APP_CLIENT:
				dbg(COMMAND_CHANNEL, "Command Type: App Chat sending a Message\n");
